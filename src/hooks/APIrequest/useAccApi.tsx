...
const useAccApi = (selectedFilter: string = "", searchValue: string="") => {
  const [users, setUsers] = useState<Infor[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

const paidUserUrl = BASE_URL + `/users?$sort[createdAt]=-1&role=paid_user${searchValue.length > 0 ? `&status=${searchValue}` : ''}`;
